...
use serde::{Serialize, Deseriablze};
...
impl Database {
	...
	fn migrate(&mut self) -> Result<()> {
		...
		self.conn.execute(
			"CREATE TABLE IF NOT EXISTS tasks (
				...
				title TEXT NOT NULL,
 				status TEXT NOT NUlL DEFUALT 'todo',
				due_date TEXT,
				tags TEXT DEFAULT '[]',
	...
	pub fn add_task(&self, title: &str, due: 
 Option<NaiveDate>, tags: Vec<String>) -> Result<u64> {
		...
		Ok(self.conn.last_insert_rowif() as u64)
	}
		...
		let sql = if show_done {
			...
			let task_iter = stmt.query_map([], |row| {
				...
				Ok(Task {
					...
					title: row.get(1)?,
					status: 
	TaskStatus::from_str(&status_str).unwrap_or(TaskStatus::Todo),
					due_date: due_str.and.then(|d| {
